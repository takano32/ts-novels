...
2. **作者が明示的に転載条件を示しているもの** — 現時点の該当は、おもちばこさんの pixiv 再掲 2 作
   (雨女・きらいなもの→ＧＷ。プロフィールに「転載前に問い合わせを」と明記)。
   許諾が取れるまで保留し、取れ次第 catalog に追加する
3. **スパム**(ラウンジ BBS のスレッド 94%、道場の汚染 13 ページ)— 除外ログを残す
4. **PII そのもの**(mailto アドレス)— 本文・投稿は載せるが、アドレスは catalog 段階で除去

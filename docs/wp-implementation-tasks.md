...
**残る指摘は軽微 3 件のみ**(下の各タスクの「気づき」。機能欠落・レイアウト崩壊は無し)。
次は ⏸ CP-4(サイト所有者レビュー)。
...
- [x] 4.3 索引ページ群: `/index/kana/{a..wa}/`・`/index/timeline/`・`/index/bunrui/`・
      ...
      - **気づき ①(軽微)**: **`/index/docs/` だけトップページからリンクされていない**。
        トップの「さくいんから探す」に出るのは kana / timeline / bunrui / vocabulary /
        osusume の **5 本**で、docs への導線が無い(URL 直打ちなら 200)
      - **気づき ②(軽微)**: `/index/docs/` の `<h1>` が汎用の**「さくいん」**。
        他の索引は h1 が固有名(「作者さくいん」「年表」等)で、`<title>` 側は
        「運営文書・資料」なので h1 だけ揃っていない
...
- [x] 4.5 検索: WP 標準検索+タクソノミーのファセット絞込テンプレート
      ...
      - **気づき ③(軽微・要判断)**: **チップを辿るとページの見出しが `<h1>学園</h1>` に変わり、
        「検索: 変身」という文脈が画面から消える**(テンプレートが検索用から
        タクソノミーアーカイブ用に切り替わるため)。1 ページの表示件数も
        検索の 10 件からアーカイブ側の件数に変わるので、**チップを押すと結果が
        増えたように見える**(実際は絞り込まれている)。機能は正しいので
        表示の問題だが、読者は戸惑いうる
- [x] 4.7 **ギャラリー区画** (設計 v1.4): `~yays/gallery/` の CG を WP に収録 (訂正: 「313」は gallery 配下の全ファイル数。画像実体は cg 136 + album 9 = 145 で、docs_build が全点収蔵)。
      ...
      - **台帳の「CG 313 点」との差**: 実際にページへ出ているのは **144 枚**。
        `~yays/gallery/` 配下の点数の数え方(サムネイルと原寸の重複、cg/ と album/ の
        区分)によるものと見られる。**数え方の確定は 👤/親の判断**(今回は
        「ページが 200 で画像が実配信される」ところまでを確認した)

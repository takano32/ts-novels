...
**状況 (2026-08-30 配備・検収。コミット `242efc62`)**: 4.1〜4.9 は**配備して動作確認済み**。
テーマ `ts-bunko` 0.1.0 を有効化し、`wp ts sync-pages`(created=4)・`wp ts sync-docs`(created=34)・
`wp ts import` を実行。**`wp ts verify` OK**(連載親への term 合併後の新期待値
**ts_genre 6182 / ts_type 5275 / ts_keyword 7128 / ts_world 495 / ts_corpus 4537 /
ts_author 4363** がすべて一致、書誌メタ・本文 status は不変)。
**主要 22 URL を巡回して全部 200・PHP エラー 0・`</html>` 到達 0 欠け**、
外部 CDN/フォントの読み込み **0 件**(設計 4.1 の要件)。
検収で出した軽微な指摘 3 件は **`82c98eba` で全て解消**(各タスクの「気づき」参照)。
**Phase 4 は完了。次は ⏸ CP-4(サイト所有者レビュー)で、ここで止まる。**
レビュー資料は **[`cp4-review.md`](cp4-review.md)** に作成済み
(レビュー用 URL 25 本・ブラウザ確認の観点・既知の未了 12 件)。

**CP-4 時点で新たに見つかった要判断 1 件**: **WordPress の初期コンテンツが公開のまま残っている** —
`https://novels.xwp.jp/sample-page/`(サンプルページ)と
`https://novels.xwp.jp/2026/07/23/hello-world/`(Hello world!)が**どちらも 200 で開ける**
(`privacy-policy` は draft なので 404 で無害)。
**本番 DB を実行セッションの判断で消さない方針**なので削除していない。
消す指示をもらえば `wp post delete` で対応する。

- [x] 4.1 ブロックテーマ `ts-bunko`(リポジトリ管理→rsync 配備): 設計書 §4 のとおり。
      ...
      **作者ページの Homepage リンクは既定で homepage_wayback、現存確認済みの移転先のみ生 URL**
      ...
      - **書誌カードのラベルは読者に見える表示文言**。「原本を見る」「初出時の姿(~yays 版)」
        ...
        (→ `docs/glossary.md`)
      - **4.1 実測 (2026-08-30)**: `wp theme activate ts-bunko` 成功、**ts-bunko 0.1.0** が有効。
        `wp rewrite flush --hard` 後の rewrite ルール **187 本**。
        話ページ `/novel/johdan-tsukinuke/19204751-tsukinuke/`(200・86,398B)で確認できたもの:
        **パンくず**(`<nav class="ts-breadcrumb">` = 城弾 › 突き抜け › 突き抜け)・
        **分類チップ**(`ts-chip` = コメディ / 変身 / ぺド / マッチョ)・**話ナビ**
        (`ts-episode-nav`)・**書誌カード**・**「無改変の原本を読む」**リンク。
        原本リンク先は **`https://takano32.github.io/ts-novels/novel/200209/19204751/tsukinuke.htm`
        で実際に 200**(死んでいる ts.novels.jp ではなく Pages を指している)。
        `style.css` **200 text/css 9,004B**。
        **外部ホストへの参照は 2 本だけで、どちらも意図的な原本アーカイブ
        (Pages の原本と感想板)。CDN・Web フォントの読み込みは 0 件**
      - **内部語の露出 0**: `/` `/about/` `/archive/` `/takedown/` `/removed/` `/index/kana/`
        のいずれにも「本館」「別館」「旧館」は**出現しない**(表示文言の規則を満たす)
      - **タクソノミーアーカイブ 5 本とも 200**: `/genre/senkihaoazuke/`・
        `/type/tshakonkainashi/`・`/keyword/toka/`・`/world/fms/`(FMS シリーズ・作品 5 本)・
        `/authors/million/`(100万Hit・作品 11 本)。`ts_corpus` だけ `rewrite=false`・
        `public=0` で URL を持たない(収蔵区分は内部用なので設計どおり)
      - **連載親への term 合併付与の確認 (コミット `1789204f`)**: `/genre/gakuen/` は
        200・34,305B で **作品 60 本**が並び、その内訳は
        **連載の親 30 / 単発 30**(例「キャリア・デイ　９７」10 話・「美少年も楽じゃない」4 話・
        「オニキス」6 話)。**子の話は 1 本も出ない**(作品単位の一覧になっている)。
        並びは **post_date の昇順 = 発表順**(先頭 1997-12-04 → 以降昇順であることを実測)。
        合併前は連載が分類アーカイブに出てこなかったので、これで書庫に連載が並ぶようになった
      - **連載の目次**: 親 `/novel/johdan-tsukinuke/` は 200 で子 2 話を列挙。
        最大の連載「戦乙女セーラ」`/novel/johdan-senotomeseera/` は
        **69 話が menu_order どおり(vol.1 → vol.2 → EPISODE2 → vol.3 …)に並ぶ**
- [x] 4.2 リーダー UI(素 JS+localStorage): 文字サイズ/行間/明朝ゴシック/ダーク/しおり/←→話ナビ。
      raw フォールバック話のみ「原本配色」トグル
      - **4.2 実測 (2026-08-30)**: 話ページに **`data-ts-toolbar` の操作列**が出力され、
        **`reader.js` が 200 application/javascript 3,837B** で配信される
        (`themes/ts-bunko/js/reader.js`)。
        **ブラウザ操作を伴う挙動(文字サイズ変更・しおりの localStorage 保存・
        ダークモード切替の実動作)はセッションから検証できない** —
        マークアップと JS の配信までを確認した。実挙動の確認は 👤 (CP-4 のレビュー時)
- [x] 4.3 索引ページ群: `/index/kana/{a..wa}/`・`/index/timeline/`・`/index/bunrui/`・
      `/index/vocabulary/`・`/index/osusume/`(`_ts_osusume(_in)` から)
      - **4.3 実測 (2026-08-30。すべて 200・PHP エラー 0)**:
        `/index/kana/`(作者さくいん・22,708B・内部リンク 21)/
        `/index/timeline/`(年表・22,539B・20)/ `/index/bunrui/`(分類さくいん・73,343B・375)/
        `/index/vocabulary/`(キーワードさくいん・115,903B・**1,032** = keyword term 数と一致)/
        `/index/osusume/`(オススメの環・207,340B・1,038)/
        `/index/docs/`(運営文書・資料・23,735B・**34** = `sync-docs` の件数と一致)
      - ~~**気づき ①**: `/index/docs/` だけトップページからリンクされていない~~
        → **`82c98eba` で解消**。トップのさくいん入口が **6 本**になり
        「運営文書・資料」が出る(作者さくいん / 年表 / 分類さくいん /
        キーワードさくいん / オススメの環 / 運営文書・資料)
      - ~~**気づき ②**: `/index/docs/` の `<h1>` が汎用の「さくいん」~~
        → **`82c98eba` で解消**。h1・`<title>` とも**「運営文書・資料」**に統一
- [x] 4.4 `/year/YYYY/` リライト(mu-plugin)とトップページ(「いまの一作」= 時間シード選出・毎時交替 (ユーザ裁定 2026-08-30、日替わりは遅い)、
      「新着」は置かない。**一覧はリロード毎ランダム — ユーザ裁定 2026-08-30**、
      定額ホストなので RAND() の負荷は許容。トップは nocache_headers でキャッシュ抑止)
      - **4.4 実測 (2026-08-30、コミット `242efc62` 配備後)**: トップ 200・24,211B。
        見出しは **「いまの一作」**(「今日の一作」の出現 **0**)・「ひらいたところから読む」・
        「さくいんから探す」。**作品リンク 13 本 = いまの一作 1 + ランダム 12**。
        **2 回続けて取得するとランダム部が入れ替わり、共通は 1 本だけ**
        (= 時間シードの「いまの一作」。リロード毎ランダムが効いている)。検索窓あり
      - `/year/YYYY/` は **`/year/2002/`(200・49,566B・作品 131 本)**・
        **`/year/1998/`(200・36,835B)** で動作を確認
- [x] 4.5 検索: WP 標準検索+タクソノミーのファセット絞込テンプレート
      (設計書の Pagefind は静的書き出し前提が v1.1 で消えたため置換 — 設計書側にも追補済み)
      - **4.5 実測 (2026-08-30)**: `/?s=変身` は 200・`<h1>検索: 変身</h1>`・
        結果 10 件(`posts_per_page=10`)・**ページ送りあり**。
        **ファセットチップ 28 個**が出て、リンクは `?s=…&ts_genre=<slug>` の形で
        検索語を保ったまま絞り込む。
      - **絞込が効いていることの確証**: `?s=ZZZQXNOTEXIST&ts_genre=gakuen` が
        **0 件 + 「該当なし」表示**になる(検索語が捨てられていれば学園の作品が並ぶはず)。
        実測: `s=変身` 10 →(1 ページ目のみ)・`s=変身&ts_genre=gakuen` **26**・
        `s=変身&ts_genre=sf` **15**・`s=変身&ts_type=henshin` **60**・
        `s=変身&ts_genre=gakuen&ts_type=henshin` **21**(複数チップの AND も効く)
      - ~~**気づき ③**: チップを辿ると h1 が「学園」に変わり検索文脈が消える~~
        → **`82c98eba` で解消**。再実測: `?s=変身` / `&ts_genre=gakuen` /
        `&ts_genre=gakuen&ts_type=henshin` の**いずれも h1 は「検索: 変身」のまま**、
        表示件数も検索側の **10 件/頁**で揃い、ページ送りも出る。
        チップ参照はファセットを足すほど増える(28 → 42 → 56 = 選択状態の表示)。
        **絞込の正しさも確認**: 素と `+学園` の結果集合は重なり 2/10 で別物、
        `+学園` の結果 **10 件すべてが実際に `gakuen` term を持つ**ことを DB 照合で確認、
        `?s=存在しない語&ts_genre=gakuen` は 0 件(検索語が捨てられていない)
- [x] 4.7 **ギャラリー区画** (設計 v1.4): `~yays/gallery/` の CG を WP に収録 (訂正: 「313」は gallery 配下の全ファイル数。画像実体は cg 136 + album 9 = 145 で、docs_build が全点収蔵)。
      作品ページの挿絵とは別区画。作者クレジットが取れるものは作者ページからも辿れるように
      - **4.7 実測 (2026-08-30)**: `/docs/gallery/` は **200・63,667B・`<img>` 144 枚**
        (`assets/annex-img/~yays/gallery/…` を 145 参照)。
        画像の実配信も確認(例 `~yays/gallery/album/images/20001207181342.jpg` =
        **200 image/jpeg 45,521B** ほか 3 点)。
        配備は deploy.sh の [5/6] が担当し、**画像は 1,196 → 1,353 点(+157)** に増えた
      - **再計測 (2026-08-30)**: `<figure>` **145** = `<img>` **145** で、
        `docs_build.py` の `gallery_images: 145` と一致。
        **前回「144 枚」と書いたのは私の数え方の誤り** — `grep` が行単位なので、
        `alt` 属性に改行を含む `<img>`(`cg_43a.gif`。alt が 3 行の説明文)を
        1 件取りこぼしていた。**ページ側は最初から 145 点で正しかった**
      - **台帳の「CG 313 点」との差は未解決**: 実際にページへ出ているのは **145 点**。
        `~yays/gallery/` 配下のサムネイル (`cg/mini/`) と原寸 (`cg/`) の重複の
        数え方によるものと見られる。**どちらが正しい点数かの確定は 👤/親の判断**
- [x] 4.8 **企画・アンソロジー** (設計 v1.4): `special/` 61 ファイル(03summer 等)を
      アンソロジー区画 (`ts_corpus=anthology`) または `ts_doc` として収録
      - **4.8 実測 (2026-08-30)**: `wp ts sync-docs` の **created=34** に
        03summer の解題 1 本が含まれる(内訳は 4.6 の項)。
        `ts_corpus=anthology` は使わず **`ts_doc` に寄せる**実装になっている
        (1.3 の実測どおり `anthology` は実データに無い語彙なので妥当)
- [x] 4.9 **文庫前史** (設計 v1.4): `~ezpe/yasai/` の静的 8 ページ(1999 年の原型サイト)を
      `ts_doc` に収録し、`/index/timeline/` から辿れるようにする
      - **4.9 実測 (2026-08-30)**: `sync-docs` の内訳に **prehistory 8** が入り、
        `/index/docs/` から 34 本すべてに辿れる。`/index/timeline/` も 200
- [x] 4.6 運営コンテンツ投入: `ts_doc` に comittee/ 21・columns/・dialy/・03summer 解題。
      ...
      **`/takedown/`(72h SLA を明文で宣言)。全ページフッタに /takedown/ へのリンク**
      ...
      - **4.6 実測 (2026-08-30)**: `wp ts sync-pages` = **created=4 updated=0 skipped=0**、
        `wp ts sync-docs` = **created=34 updated=0 skipped=0**
        (comittee 21 + columns 2 + dialy 1 + prehistory 8 + gallery 1 + 解題 1)。
        固定ページは 4 枚とも 200 で、h1 は
        **`/about/` 「このサイトについて」/ `/archive/` 「原本アーカイブ」/
        `/takedown/` 「掲載の取り下げについて」/ `/removed/` 「取り下げ済みの一覧」**。
        `/about/` は八重洲メディアリサーチの持ち主の依頼によるリブートである旨を明記、
        `/removed/` は「現在、取り下げられた作品はありません。」を表示。
        **全ページのフッタに `/takedown/` へのリンクがある**ことを確認
        (`/takedown/` 自身は本文中の分と合わせて 2 箇所)。
        **「本館」「別館」「旧館」の露出は 4 枚とも 0**

## 🔍 公開前レビュー (2026-08-30 夜。CP-4 の前に実施)

Phase 2〜4 の Fable 実装コードを 6 視点 (importer 冪等性 / 変換の盲点 / エスケープ・XSS /
テンプレート分岐 / JS 互換 / 運用安全) でレビューし、指摘ごとに独立の反証を通して 15 件を確定。
**すべて修正・配備・実地検証済み** (コミット `f04d7a5e`)。詳細は `docs/cp4-review.md` §5。

重大だったもの: ①**取り下げ機構が実質無効** (denylist の実スキーマ `entries[].target.{kind,value}`
を読んでおらず 0 件適用で成功終了 + 子投稿へ波及しない) → 作り直し、実測で
work=親+69 話・author=37 作・reinstated 除外を確認 / ②**本文 47 本に実アドレス 26 種**
(payload_build にアドレス除去が無かった。docs_build にはあった) → 全経路で除去 +
出荷前の自己検査 (残存 1 本でも exit 1) / ③**/index/kana/ が 324 名中 21 名**
(見出しキー 'あ' と実値 'あ行'/'K' の不一致) → 324 名全表示 / ④**閉じない table が
後続ブロックを呑む** (1 話で 636 段落が引用内に吸収) → 引用内 table 除去・段落内未閉 table は
wp:html 化 / ⑤`?ts_index[]=x` で **PHP 8 TypeError → 500** → query var を許可リストで正規化。

性能: オススメの環が毎リクエスト 1,300 本超のクエリ → 全件マップ 2 クエリ (リンクも
1,038 → 1,756 に増加。連載トビラ参照 256 件が解決できるようになったため)。
一覧カードの話数は `_ts_episode_count` メタから (旧: カードごとに get_children)。

配備: PHP 構文検査を**配備の前**に移し全 .php を対象に。.htaccess の原本は `.orig` に 1 度だけ保存。

## 🔍 公開前レビュー 2 回目 (2026-08-31 未明)

catalog パイプライン / 本文変換 / 読者視点の実サイト / 文書の正確さ / プライバシーを監査し
25 件を確定。詳細と対処は `docs/cp4-review.md` §6。特筆すべきもの:

- **取り下げ済み作品 9 件が再公開されていた** — 目録の「作者の要望により公開停止」告知を
  パーサが拾っておらず、最長 152KB の全文が読める状態だった。18 URL を 410 化 +
  /removed/ 掲示 + 検査 2 本 (`公開停止の告知を検出 == 9 件` / `停止告知のある話が
  denylist に載っている`) を make check に追加して再発を止めた (TD-2026-0001〜0009)
- **オフ会レポートの顔写真** — 私人 15 枚。投稿と画像の両方を 410 (TD-2026-0010〜0013)。
  掲示は保留、扱いは 👤 判断
- **原本アーカイブ側の第三者 PII** — 読者のメールアドレス・投稿者 IP。
  未対応・判断待ち → `docs/annex-privacy-decision.md`
- 読了目安がバイト数由来で破綻 (289 ページ) / 索引の件数が実際の 3 倍 /
  本文中のリンクが 472 ページで死んでいた → いずれも修正
- **手順書に破壊的な誤り**: `--bodies=../bodies/` は全話をプレースホルダにする。
  `deploy.sh -n` は存在せず何も配備されない → 訂正済み
...
- [ ] 👤 CP-4 **サイト所有者のレビュー**。Phase 4 の全タスク完了後、実行セッションは
      **Phase 5 以降に進まず**、次を揃えて報告して止まること:
      1. レビュー用 URL 一覧(トップ / 作品 3 例 / 話 3 例〔MD・raw フォールバック・画像作品を各 1〕/
         作者 2 例 / 各索引 / 検索。v1.5: 公開状態のままレビュー — noindex は /boards/ /dojo/ の恒久層のみ)
      2. その時点の `wp option get ts_catalog_commit` の値(NG 報告の基準点)
      3. 既知の未了・妥協点の一覧
      NG の出し方と再走手順は `docs/rebuild-runbook.md`(§3 と §2)。
      レビュー通過の連絡があってから Phase 5 へ。
      - **資料は用意済み (2026-08-30) → [`docs/cp4-review.md`](cp4-review.md)**。
        レビュー用 URL **25 本(全て 200 と中身を実測確認済み)**・
        ブラウザでないと確かめられない観点 4 点・**既知の未了と妥協点 12 件**・
        NG の出し方を載せてある。
        **基準点 `ts_catalog_commit` = `85cc9bf7dd436021dc5634fb05d07cb36d177814`**
        (配備コードは `82c98eba`。`85cc9bf7` 以降はテーマと mu-plugin のみの変更で
        **catalog の中身は不変**なのでこの値が内容の基準点として正しい)。
      - **公開設定の現況 (v1.5 どおり)**: `/` `/novel/…` `/docs/gallery/` `/index/…` の
        いずれにも **`X-Robots-Tag` も meta robots も出ていない = インデックス可**。
        `robots.txt` は AI 学習クローラ(GPTBot・ClaudeBot・CCBot・PerplexityBot ほか)を
        Disallow し、一般検索エンジンは止めていない。
        恒久 noindex 層 `/boards/` `/dojo/` は **Phase 6 で作るのでまだ存在しない**
      - **⏸ ここで停止中。実行セッションは Phase 5 に進まない。**

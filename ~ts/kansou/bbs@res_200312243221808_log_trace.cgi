...
<input type=hidden name="nj" value="tRogLo2pcV">
...
内容 ＼ 題名 <input type=text name="subject" size=45 maxlength=80 value="Re2:よんだっつーの"><br>
<textarea name="value" rows=5 cols=80 wrap=soft>▼ ｍｋ８４２６＠自宅さん
&gt; ▼ Kardyさん
&gt; &gt; ＞キロ28‐2833
&gt; &gt; 随分年代ものの天使ですな。
&gt; &gt; 2000番台ってのは…寒冷地改造済の証ですか？(w
&gt; 
&gt; 2000番台は発電用エンジン搭載の証です。自車を含め3両まで給電することができます。
&gt; 同系列のキハ58が走行用エンジン2台搭載で発電用エンジンを搭載できなかったため、このような給電形態がとられました（あと、普通車で走行用エンジンが1台のキハ28も同様に発電用エンジン搭載が2000番台です）。ですので、キハ58しかいない編成では屋根上にクーラが付いていても使用することはできません。また、後に登場したキハ65（走行用大出力エンジン1台と発電用エンジン1台搭載）の位置づけは、走行出力を確保しながら発電容量も確保するというものでありました。
&gt; なお、2000番台というのは便宜上の表現であり、正確には全車が改造車なので、元番号に2000をプラス、というのが正確なところです。
&gt; 
&gt; 北海道用のキハ58系列はキハ27・56・キロ26になります。
&gt; 
&gt; 以上、「鉄」講座でした（爆）。</textarea><br>

...
<input type=hidden name="nj" value="6MznIobO3w">
<input type=hidden name="res" value="1905">
<h4 align=center>▽ 記事番号 1905 への返信 ▽</h4><table border=0 cellpadding=2 cellspacing=0><tr><td align=left nowrap>
名前 <input type=text name="name" size=45 value="" maxlength=64><br>
E-Mail <input type=text name="email" size=45 value="" maxlength=200><br>
URL <input type=text name="url" size=45 value="" maxlength=200><br>
</td></tr>
<tr><td align=left nowrap>
内容 ＼ 題名 <input type=text name="subject" size=45 maxlength=80 value="Re:企画「Ｔ'ｓＨｅａｒｔ（仮）」に関する提案（１）"><br>
<textarea name="value" rows=5 cols=80 wrap=soft>▼ 南文堂さん
&gt; 　ＢＡＦさん、ＭＯＮＤＯさん、無糖さんのご意見を拝見して、私も考えてみました。
&gt; 
&gt; 　掲示板はそこそこまとまった文章を手軽に掲載できる利点がありますが、即時性においてはチャットなどに比べて少し劣る面をもちます。しかし、掲示板でチャット並にやりとりされると閲覧が苦痛になるので、劣る即時性が利点ともいえなくはないです。
&gt; 　その利点が前回のキャノンボールで失われたことが混乱と読者参加者離れが進んだ原因と私は分析しています。掲示板のチャット状態を回避するために、すごろくのマスというか、漫画のコマというか、チェックポイントというのを作り、物語が進みすぎるのを回避する手法が今回の企画システムの肝であるとおもいます。（つまり、ＢＡＦさんの一日日記形式、ＭＯＮＤＯさんのイベントしばりがそれにあたります）
&gt; 　しかし、現在でている企画案は、そのマスが広い（長い）と私は思います。そこで、もっと小さなマスにする事を提案します。
&gt; 　さらに原案ではゲームマスターを必要としていますが、運営側はオフィシャルの流れだけを管理して、参加者はその下でパーソナルな遊びに興じる。これなら、自由度は高く保たれ、比較的管理は楽になるのではないでしょうか？
&gt; 
&gt; 　わかりにくいので具体的な例を示します。
&gt; 
&gt; イベント１：入学式
&gt; ＜オフィシャル　選択肢＞
&gt; 　おめでとうございます。あなたはＴＳ学園に入学しました。今日はその入学式。これからの学園生活をどうぞ楽しんでください。
&gt; 　さて、あなたは入学式の会場に入りました。席は自由に座っていいようです。あなたはどこに座りますか？
&gt; １）右側　２）左側　３）中央　４）壇上
&gt; ＜参加者　アクション＞
&gt; 　俺は（中略）というわけで、右側に座る事になった。
&gt; ＜オフィシャル　選択結果＞
&gt; １）右側を選択した人。男性はＴＳします。女性はそのままです。
&gt; ２）左側を選択した人。男性はそのまま。女性はＴＳします。
&gt; ３）中央を選択した人。変化なし。
&gt; ４）壇上を選択した人。ロザリオを渡されて、生徒会候補生として働く事になりました。
&gt; ＜参加者　リアクション＞
&gt; 　どういうことだ？　おれの身体が突然（以下略）。
&gt; 
&gt; イベント２：クラス分け
&gt; ――（以下略）――
&gt; 
...
&gt; ＜オフィシャル　選択結果＞
&gt; ＜参加者　リアクション＞
&gt; 　という区切りをはっきりとしておく。これを一定周期にすれば、参加する側も楽になると思います。
&gt; 　例えば、日曜日夜に＜選択肢＞発表、木曜日夜に＜選択結果＞発表といった具合にすれば、月火水木の間に＜アクション＞を選択、金土日に＜リアクション＞を書き込めばよくなります。
&gt; 
&gt; （２）に続く</textarea><br>
pre<input type=checkbox name="pre" value="1"> 
link<input type=checkbox name="link" value="1" checked> 
削除キー <input type=password name="pwd" size=10 value=""> 入力コード cSzF8V  <input type=password name="rcode" size=10 value=""> <input type=hidden name="hrcode" size=10 value="cSzF8V"> <input type=submit value="書き込む"><input type=reset value="書き直し"><br></td></tr></table>
</td></tr></table><br>
</form>
